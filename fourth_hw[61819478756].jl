...
function minimize(objective::Function, 
    ...
    for iter in 1:max_iter
        t = objective(x)
        p = [4 * x[1] * (x[1]^2 + x[2] - 11) + 2 * (x[1] + x[2]^2 - 7),
                    2 * (x[1]^2 + x[2] - 11) + 4 * x[2] * (x[1] + x[2]^2 - 7)]
        penalties = constraints(x)
        firstpen, secondpen = penalties
        firstpen = 2 * (norm(x) - 3.6) * x / norm(x)
        secondpen = 2 * (x[1] + x[2] - 5) * [1, 1]
        loss = p + λ * firstpen + μ * secondpen
        x -= lr * loss
        if norm(loss) < stopping_criterion
            break
        end
    end
    return x
